...
23. **`gh api --slurp --jq` incompatibility**: newer `gh` CLI versions reject `--slurp` combined with `--jq`. The GitHub poller (`github_poller.go`) uses this combination for paginated comment fetching, causing monitor warnings. Needs fix to use `--paginate --jq` without `--slurp`.
