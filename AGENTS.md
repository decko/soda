...
39. **Arapuca wrapper vs libarapuca distinction**: `arapuca-devel` (BuildRequires) provides `libarapuca.a` for CGO linking at build time. The `arapuca` package (Requires) provides the `/usr/bin/arapuca` wrapper binary needed at runtime for Landlock/seccomp enforcement. `sandbox.WrapperBinaryPath()` calls `arapuca.WrapperPath()` to locate it. The wrapper is checked in three places: `soda doctor` (checkArapucaWrapper), `soda validate` (Stage 10), and `soda run` preflight (when `sandbox.enabled: true`). Install with `sudo dnf install arapuca`. **Known gap**: go-arapuca v0.2.0 has no `LibraryVersion()` API, so there is no version-comparison warning between the wrapper binary and libarapuca — this requires upstream changes.
