...
Key: 375
Summary: feat(engine): structured error messages for common failures
...
- **Cycle 1** (review, verdict: rework): [major] internal/pipeline/review.go:423 — Prompt load/render errors and retry exhaustion in runReviewer still use fmt.Errorf instead of the new PromptError and RetriesExhaustedError types. Lines 423, 434, and 552 produce unstructured errors for the same failure modes that engine.go now wraps in structured types. Callers using errors.As to match PromptError or RetriesExhaustedError will miss errors from parallel-review phases.; [major] internal/pipeline/monitor_poll.go:723 — Prompt load (line 723) and render (line 728) errors in runMonitorIteration still use fmt.Errorf instead of the new PromptError type, creating inconsistency with engine.go's runPhase.
...
### Finding 1 of 4: ai-harness
- [major] internal/pipeline/review.go:552 — RetriesExhaustedError.Phase stores reviewer name, not pipeline phase name. The struct field is documented as the phase name and is used consistently as such in engine.go:runWithRetry (line 807), but in review.go:runReviewerWithRetry it receives `reviewer.Name` (e.g. 'go-specialist') instead of `phase.Name` (e.g. 'review'). Consumers doing errors.As to extract Phase will get inconsistent semantics depending on whether the error originated from a phase or a reviewer.
  Suggestion: Either (a) rename the field to `Subject` to be agnostic, or (b) use `phase.Name` here and add a separate `Reviewer` field: `&RetriesExhaustedError{Phase: phase.Name, Reviewer: reviewer.Name, ...}`. Option (b) preserves the original `reviewer.Name` context from the old fmt.Errorf while keeping Phase semantics consistent.
  Relevant code:
```
package pipeline
...
### Finding 2 of 4: ai-harness
- [major] internal/pipeline/review.go:423 — PromptError for reviewer load/render loses reviewer identity. The original fmt.Errorf included the reviewer template path (`reviewer.Prompt`) for load errors and `reviewer.Name` for render errors. The new PromptError uses `Phase: phase.Name` for both, so when multiple reviewers run in parallel and one fails, the error message no longer identifies which reviewer's prompt failed — only the parent phase name (e.g. 'review').
  Suggestion: Add a `Reviewer string` field to PromptError (or a generic `Subject string` field) and populate it with `reviewer.Name` in the review path. Update the Error() format to include it when non-empty: `engine: prompt load for phase review (reviewer go-specialist): ...`
  Relevant code:
```
package pipeline
...
### Finding 3 of 4: ai-harness
- [major] internal/pipeline/engine.go:1247 — emitPhaseFailed flattens structured errors to a plain string. The new structured error types (PromptError, RetriesExhaustedError) carry machine-readable fields (Phase, Category, Attempts, Operation), but emitPhaseFailed writes only `phaseErr.Error()` as a flat string into events.jsonl. The ticket's acceptance criteria require structured error metadata (including suggestions) to appear in events.jsonl. This is not a regression (the old code also flattened), but the structured types provide an opportunity that is missed.
  Suggestion: In emitPhaseFailed, use errors.As to detect structured error types and propagate their fields into the event Data map. For example: if a RetriesExhaustedError is found, add `data["category"] = re.Category; data["attempts"] = re.Attempts`. This is also the integration point for the suggestion enrichment the ticket requires.
  Relevant code:
```
}
...
### Finding 4 of 4: ai-harness
- [critical] internal/pipeline/errors.go:1 — Ticket acceptance criteria not addressed: the ticket requires a data-driven enrichment catalog mapping the 7 transient error reasons from classifyExitError (oom, signal, rate_limit, timeout, overloaded, connection, unknown) to user-facing suggestions. No such catalog exists. The 4 new structured error types (PhaseNotFoundError, RetriesExhaustedError, WorktreeError, PromptError) are internal engine errors unrelated to transient CLI failures. The classifyExitError function (internal/claude/runner.go:88) and its TransientError type (internal/claude/errors.go:7) are completely untouched.
  Suggestion: Implement the enrichment layer: (1) Create a suggestion catalog mapping TransientError.Reason values to user-facing suggestions (e.g. oom→'Reduce context size or increase memory limits'). (2) Add enrichment in the retry/failure path where TransientError is unwrapped — either in classifyError or emitPhaseFailed. (3) Surface the suggestion field in events.jsonl Data map. (4) Ensure suggestions are excluded from retry prompt text (engine.go:828,839). (5) Add tests for all 7 reasons + unknown fallback.
  Relevant code:
```
package pipeline
...
You are working in a git worktree at: /home/ddebrito/dev/soda/.worktrees/soda/375
Branch: soda/375
Base: main
...
5. **Run the formatter** if configured: `gofmt -w .`
6. **Run the tests** if configured: `go test ./...`
